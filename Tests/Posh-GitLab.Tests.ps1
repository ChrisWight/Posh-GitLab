[string]           $projectDirectoryName = 'Posh-GitLab'
...
Describe $testFile.Name {
    ...
    $public = @( Get-ChildItem -Path "${projectDirectory}\Public\*.ps1" -Recurse -ErrorAction SilentlyContinue )
    ...
    $private = @( Get-ChildItem -Path "${projectDirectory}\Private\*.ps1" -Recurse -ErrorAction SilentlyContinue )
